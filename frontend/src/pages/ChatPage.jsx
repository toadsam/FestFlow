  ...
  return (
    <section className="cyber-page pt-4 space-y-4">
      ...
      <div className="rounded-2xl bg-slate-900 text-white p-4 min-h-44 space-y-3">
        {messages.slice(-10).map((message, idx) => (
          ...
          >
            ...
            <p
              className={`inline-block max-w-[90%] rounded-lg px-3 py-2 ${message.role === "user" ? "bg-teal-600" : "bg-slate-700"}`}
            >
              {message.text}
            </p>
          </div>
      ...
      <form onSubmit={handleSubmit} className="flex gap-2">
        ...
        />
        <button
          className="rounded-xl bg-amber-500 text-white px-4 font-semibold"
          type="submit"
          disabled={loading}
        >
          전송
