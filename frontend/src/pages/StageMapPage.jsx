  ...
  return (
    <section className="pt-4 space-y-3">
      ...
      <article className="rounded-xl border border-slate-200 bg-white p-3 space-y-2">
        <div className="grid grid-cols-3 gap-2 text-center">
          <div className="rounded bg-teal-50 p-2">
            ...
            <p className="text-lg font-bold text-teal-800">{theater?.crowdCount ?? 0}명</p>
          </div>
          <div className="rounded bg-amber-50 p-2">
            <p className="text-[10px] text-amber-700">혼잡도</p>
            <p className="text-sm font-bold text-amber-800">{theater?.level || '-'}</p>
          </div>
          <div className="rounded bg-indigo-50 p-2">
            <p className="text-[10px] text-indigo-700">업데이트</p>
            <p className="text-xs font-bold text-indigo-800">{formatUpdatedAt(stageData?.updatedAt)}</p>
          </div>
        </div>
          ...
          {theater && (
            <>
              ...
              <CircleMarker
                center={[theater.latitude, theater.longitude]}
                radius={pulseRadius}
                pathOptions={{ color: '#ffffff', fillColor: style.fill, fillOpacity: 0.95, weight: 2 }}
              >
                <Popup>
      ...
      <article className="rounded-xl border border-slate-200 bg-white p-3">
        <p className="text-sm font-semibold text-slate-800">노천극장 혼잡 게이지</p>
        <p className="mt-1 text-xs text-slate-600">기준 수용치 {theater?.capacityHint ?? 0}명 대비</p>
      ...
      {loading && <p className="text-sm text-slate-600">노천극장 데이터를 불러오는 중...</p>}
