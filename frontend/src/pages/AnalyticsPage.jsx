  ...
  return (
    <section className="pt-4 space-y-4">
      <div className="flex items-center justify-between">
        ...
        <button type="button" onClick={loadAnalytics} className="text-xs rounded-lg border px-2 py-1">
          새로고침
        </button>
      </div>
      ...
      <article className="rounded-xl border border-slate-200 bg-white p-3">
        ...
        <div className="mt-2 space-y-1.5">
          {popular.length === 0 && <p className="text-xs text-slate-500">집계 데이터가 없습니다.</p>}
      ...
      <article className="rounded-xl border border-slate-200 bg-white p-3">
        ...
        <div className="mt-2 grid grid-cols-2 gap-2">
          {heatmap.length === 0 && <p className="text-xs text-slate-500">집계 데이터가 없습니다.</p>}
