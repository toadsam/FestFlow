    ...
    <section className="cyber-page pt-4 space-y-4">
      <div className="flex items-center justify-between">
        ...
        <button
          ...
          className="text-xs rounded-lg border px-2 py-1 min-h-11"
        >
          새로고침
        </button>
      </div>
          ...
          {traffic.map((item) => (
            <div key={item.hour} className="min-w-7 text-center">
              <div
                ...
                style={{
                  height: `${Math.max(6, (item.count / trafficMax) * 100)}px`,
                }}
        ...
        <div className="mt-2 grid grid-cols-2 gap-2">
          ...
          {heatmap.slice(0, 12).map((point) => (
            ...
            >
              <div className="flex items-center justify-between">
                <span
                  className={`inline-block w-2.5 h-2.5 rounded-full ${intensityClass(point.intensity)}`}
                />
